...
import {
  DollarSign, TrendingUp, Users, FileDown, FileSpreadsheet,
  CalendarIcon, ShoppingCart, Wallet, PiggyBank
} from "lucide-react";
...
export function MonitorAccounting() {
  ...
  const { data: voucherSales = [] } = useQuery({
    queryKey: ['hotspot-accounting-sales', mikrotikId, dateKey],
    queryFn: async () => {
  ...
  const { data: activeVoucherSales = [] } = useQuery({
    queryKey: ['hotspot-accounting-active', mikrotikId, dateKey],
    queryFn: async () => {
      if (!mikrotikId) return [];
      const { data, error } = await supabase
        .from('vouchers')
  ...
  const { data: resellers = [] } = useQuery({
    queryKey: ['hotspot-accounting-resellers', mikrotikId],
    queryFn: async () => {
      if (!mikrotikId) return [];
      const { data, error } = await supabase.from('reseller_assignments').select('*').eq('mikrotik_id', mikrotikId);
      if (error) throw error;
      return data || [];
    },
    enabled: !!mikrotikId,
  ...
  const { data: resellerProfiles = [] } = useQuery({
    queryKey: ['hotspot-accounting-reseller-profiles', resellers],
    queryFn: async () => {
  ...
  const allSales = useMemo(() => [...voucherSales, ...activeVoucherSales], [voucherSales, activeVoucherSales]);

  const balance = useMemo(() => {
    const totalIncome = allSales.reduce((s: number, v: any) => s + Number(v.price || 0), 0);
    return { totalIncome, count: allSales.length };
  }, [allSales]);
  ...
  const monthlyData = useMemo(() => {
    ...
    return months.map(date => {
      ...
      return {
        ...
        vouchers: mv.reduce((s: number, v: any) => s + Number(v.price || 0), 0),
      };
    });
  ...
  const resellerReport = useMemo(() => {
    return resellers.map((r: any) => {
      ...
      return {
        id: r.id, name: profile?.full_name || profile?.email || 'Sin nombre',
        email: profile?.email || '', salesCount: sales.length, totalSales,
        commissionRate: Number(r.commission_percentage), commission, netIncome: totalSales - commission,
      };
    });
  }, [resellers, allSales, resellerProfiles]);

  const recentMovements = useMemo(() => {
    return allSales.map((v: any) => ({
      date: v.sold_at, type: 'voucher',
      description: `Voucher ${v.voucher_code || v.code}`,
      amount: Number(v.price || 0), profile: v.profile || '-',
    })).sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, 30);
  }, [allSales]);

  const exportCSV = () => {
    const rows = recentMovements.map(m => [
      format(new Date(m.date), "dd/MM/yyyy HH:mm"), m.description, m.profile, m.amount.toFixed(2),
    ]);
    const lines = [
      `Contabilidad Hotspot - ${format(startDate, 'dd/MM/yyyy')} a ${format(endDate, 'dd/MM/yyyy')}`,
      "", `Total Ingresos: $${balance.totalIncome.toLocaleString()}`, `Total Ventas: ${balance.count}`, "",
      ...(resellerReport.length > 0 ? [
        "--- Revendedores ---", "Nombre,Ventas,Total,Comisión %,Comisión $,Neto",
      ...
      "--- Ventas ---", "Fecha,Voucher,Perfil,Monto",
      ...rows.map(r => r.join(",")),
    ];
    const blob = new Blob(["\ufeff" + lines.join("\n")], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url;
    a.download = `contabilidad_hotspot_${format(startDate, 'yyyy-MM-dd')}.csv`; a.click();
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(10);
    doc.text(`Total Ingresos Vouchers: $${balance.totalIncome.toLocaleString()}`, 20, y); y += 7;
    doc.text(`Vouchers Vendidos: ${balance.count}`, 20, y); y += 12;

    if (resellerReport.length > 0) {
      doc.setFontSize(12); doc.text("Revendedores", 20, y); y += 8;
      doc.setFontSize(9);
      resellerReport.forEach(r => {
        if (y > 270) { doc.addPage(); y = 20; }
        doc.text(`${r.name}: ${r.salesCount} ventas, $${r.totalSales.toFixed(0)} | Com: $${r.commission.toFixed(0)} (${r.commissionRate}%) | Neto: $${r.netIncome.toFixed(0)}`, 20, y);
        y += 6;
      });
      y += 6;
    }
    ...
    doc.save(`contabilidad_hotspot_${format(startDate, 'yyyy-MM-dd')}.pdf`);
    ...
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold">Contabilidad Hotspot</h2>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" size="sm" className="h-7 text-xs">
                <CalendarIcon className="mr-1 h-3 w-3" />{format(startDate, "dd/MM/yy")}
              </Button>
            </PopoverTrigger>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" size="sm" className="h-7 text-xs">
                <CalendarIcon className="mr-1 h-3 w-3" />{format(endDate, "dd/MM/yy")}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="start">
        ...
        <Card className="border-l-4 border-l-primary">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-medium text-muted-foreground">Total Ingresos</p>
              <DollarSign className="h-3.5 w-3.5 text-primary" />
            </div>
            <p className="text-lg font-bold mt-1">${balance.totalIncome.toLocaleString()}</p>
            <p className="text-[10px] text-muted-foreground">{balance.count} ventas</p>
          </CardContent>
        </Card>
        <Card className="border-l-4 border-l-success">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-medium text-muted-foreground">Vouchers Vendidos</p>
              <ShoppingCart className="h-3.5 w-3.5 text-success" />
            </div>
            <p className="text-lg font-bold mt-1">{balance.count}</p>
          </CardContent>
        </Card>
        <Card className="border-l-4 border-l-warning">
          <CardContent className="p-3">
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-medium text-muted-foreground">Revendedores</p>
              <Users className="h-3.5 w-3.5 text-warning" />
            </div>
            <p className="text-lg font-bold mt-1">{resellers.length}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="h-8">
          ...
          <TabsTrigger value="resellers" className="text-xs">Revendedores</TabsTrigger>
          <TabsTrigger value="movements" className="text-xs">Ventas</TabsTrigger>
        </TabsList>

        <TabsContent value="balance" className="mt-3">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">Ingresos por Vouchers</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          {resellerReport.length > 0 ? (
                    ...
                    <CardContent className="p-3">
                      ...
                      <div className="grid grid-cols-2 gap-1.5 text-xs">
                        ...
                        <div className="p-1.5 bg-warning/10 rounded">
                          <p className="text-[10px] text-muted-foreground">Comisión ({r.commissionRate}%)</p>
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-8 text-muted-foreground">
                ...
                <p className="text-sm">No hay revendedores asignados</p>
              </CardContent>
            </Card>
          )}
        </TabsContent>
                  ...
                  <TableBody>
                    {recentMovements.length > 0 ? recentMovements.map((m, i) => (
                      <TableRow key={i}>
                        ...
                        <TableCell className="text-right text-xs font-medium text-success">+${m.amount.toLocaleString()}</TableCell>
                      </TableRow>
                    )) : (
