...
export function MonitorVouchers() {
  ...
  const { data: deviceInfo } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from('mikrotik_devices').select('hotspot_url').eq('id', selectedMikrotik).single();
  ...
  const handleSync = () => {
    if (!selectedMikrotik) { toast.error('No hay dispositivo conectado'); return; }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold">Vouchers</h2>
        </div>
        <div className="flex gap-2">
          {selectedVouchers.length > 0 && (
            <Button size="sm" variant="outline" onClick={handleBatchPrint}>
              <Printer className="h-3.5 w-3.5 mr-1" /> Imprimir ({selectedVouchers.length})
            </Button>
          )}
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="space-y-1">
              ...
              <Input type="number" min="1" max="100" value={voucherCount} onChange={(e) => setVoucherCount(parseInt(e.target.value) || 1)} className="h-8 text-xs" />
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              ...
              <Input type="number" value={price} readOnly className="h-8 text-xs" />
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label className="text-xs">Nombre Negocio</Label>
              <Input value={businessName} onChange={(e) => setBusinessName(e.target.value)} className="h-8 text-xs" />
            </div>
            <div className="space-y-1">
              <Label className="text-xs">Logo</Label>
              <input ref={fileInputRef} type="file" accept="image/*" onChange={handleLogoUpload} className="hidden" />
              <Button variant="outline" size="sm" onClick={() => fileInputRef.current?.click()} className="w-full h-8 text-xs">
                <Upload className="h-3.5 w-3.5 mr-1" /> {logo ? 'Cambiar' : 'Subir'}
              </Button>
            </div>
          </div>
