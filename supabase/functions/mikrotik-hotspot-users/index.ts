...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { host, username, password, port, action, userId, userData, profileData, mikrotikId } = await req.json();

    console.log(`User ${user.id} - MikroTik Hotspot Users - Action: ${action}`);
