...
echo ""
echo -e "${YELLOW}⚠ MikroTik por WireGuard (tras reinstalar cambian las llaves):${NC}"
echo "  1) En la MikroTik:  :put [/interface wireguard get wg-omnisync public-key]"
echo "  2) En el VPS, registra esa llave de forma persistente:"
echo "     bash $INSTALL_DIR/vps-stack/wg-peer-sync.sh <PUBKEY> none 10.13.13.2 10.82.0.0/21"
echo "  3) Verifica en la MikroTik: ping 10.13.13.1 count=4"
echo ""
echo -e "${YELLOW}TR-069 en las ONUs (GenieACS):${NC}"
echo "  ACS URL:            http://10.13.13.1:7547/   (por WireGuard, recomendado)"
...
echo "  STUN:               Disabled (no hace falta con WireGuard)"
