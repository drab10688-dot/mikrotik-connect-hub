...
# 5d) Instalar tema OmniSync opcional (desactivado por defecto para evitar fallos de compatibilidad)
THEME_NAME="${NUXBILL_THEME:-}"
THEME_SRC="/opt/omnisync-theme"
THEME_DST="$NUXROOT/ui/themes/${THEME_NAME}"
if [ -n "$THEME_NAME" ] && [ -d "$THEME_SRC" ]; then
  ...
  log "Tema ${THEME_NAME} instalado ✓ → ui/themes/${THEME_NAME}/"
  ...
  if [ "$CONNECTED" = true ] && [ "$SCHEMA_FOUND" = true ]; then
    php -r "
      \$c = new mysqli('${DB_HOST}', '${DB_USER}', '${DB_PASS}', '${DB_NAME}');
      if (!\$c->connect_error) {
        \$theme = \$c->real_escape_string('${THEME_NAME}');
        \$c->query(\"INSERT INTO tbl_appconfig (setting, value) VALUES ('theme', '\$theme') ON DUPLICATE KEY UPDATE value='\$theme'\");
        echo 'Tema activado en DB ✓';
        \$c->close();
      }
    " 2>/dev/null || true
  fi
  ...
  if [ "$CONNECTED" = true ] && [ "$SCHEMA_FOUND" = true ]; then
    php -r "
      \$c = new mysqli('${DB_HOST}', '${DB_USER}', '${DB_PASS}', '${DB_NAME}');
      if (!\$c->connect_error) {
        \$c->query(\"INSERT INTO tbl_appconfig (setting, value) VALUES ('theme', '') ON DUPLICATE KEY UPDATE value=''\");
        echo 'Tema por defecto activado en DB ✓';
        \$c->close();
      }
    " 2>/dev/null || true
  fi
  log "Tema personalizado desactivado (NUXBILL_THEME vacío) ✓"
fi
