...
// ── IMPORTANT: Specific routes MUST come before /:mikrotikId to avoid UUID collision ──

// Paid history (GET with query params)
...
invoicesRouter.post('/generate-single', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const now = new Date();
    const invoiceNumber = `INV-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}-${clientRows[0]?.username || 'CLI'}-${Date.now().toString(36)}`;
...
// ── Routes with :mikrotikId param (MUST be after specific routes) ──

// List invoices
invoicesRouter.get('/:mikrotikId([0-9a-fA-F-]{36})', async (req: AuthRequest, res: Response) => {
  try {
...
// Create invoice
invoicesRouter.post('/:mikrotikId([0-9a-fA-F-]{36})', async (req: AuthRequest, res: Response) => {
  try {
    const { mikrotikId } = req.params;
    const hasAccess = await verifyDeviceAccess(req.userId!, req.userRole!, mikrotikId);
    if (!hasAccess) return res.status(403).json({ error: 'Sin acceso' });
...
// Record payment
invoicesRouter.post('/:mikrotikId([0-9a-fA-F-]{36})/pay/:invoiceId', async (req: AuthRequest, res: Response) => {
  try {
...
// Delete invoice
invoicesRouter.delete('/:mikrotikId([0-9a-fA-F-]{36})/:invoiceId', async (req: AuthRequest, res: Response) => {
  try {
    const { mikrotikId, invoiceId } = req.params;
    const hasAccess = await verifyDeviceAccess(req.userId!, req.userRole!, mikrotikId);
    if (!hasAccess) return res.status(403).json({ error: 'Sin acceso' });
